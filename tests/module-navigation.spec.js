...
test('proposal demo route enters target module from its top', async ({ page }) => {
  await page.goto('/')

  await page.locator('.proposal-route-step').filter({ hasText: '补贴匹配' }).getByRole('button').click()
  ...
  expect(headingTop).toBeGreaterThanOrEqual(stickyBottom - 1)
})
