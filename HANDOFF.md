...
- 完成合同综合审查：用户点击“开始综合审查”即主动发起本地规则与 AI 协作；本地规则先在后台生成风险线索，AI 再结合双重脱敏合同文字、线索和服务端知识库核验补漏，逐字证据校验后一次性展示合并报告；AI 失败、取消或长合同超限时回退本地结果。
- 合同逐字证据不写入内存或 Redis 成功结果缓存，仅保存不含正文的短期完成标记；取消综合审查会通知服务端中止模型请求并回滚额度，长合同个别分段失败时保留成功分段并明确提示覆盖范围。
...
- 完成多房源档案 MVP：首页可新建、切换、重命名和删除房源；合同、验房、证据包、补贴和 AI 对话按当前房源隔离。
- 完成证据包完整性证明 MVP：导出 ZIP 附带证据包清单.json，对合同快照、照片和附件生成 SHA-256 指纹，缺失文件写入 skipped 清单。
- 完成真实 AI/OCR 冒烟监测：GET /api/ai/smoke 可用极小内置样本验证线上模型和 OCR 链路；该接口属于受保护诊断接口，不进入用户正常流程。
...
3. 微信公众平台后台的隐私保护指引需增加“AI 合同全文复核”用途，披露点击综合审查会把双重脱敏合同文字发送至租小审服务端和模型服务商。
...
最近一次本地验证结果（具体数字以 `npm run verify:miniapp` 当前输出为准）：

- ESLint：通过
- Web 构建：通过
- RAG 评测：56/56
- 合同审查基准：11 份合同，162/162 预期风险召回
- Playwright E2E：22/22
- 小程序核心检查：20 findings / score 88
- 小程序可靠性：170/170
- 小程序 AI 安全：49/49
- 小程序用量存储：11/11
- 合同导入：9/9
...
3. 在微信公众平台后台隐私保护指引增加“AI 合同全文复核”，说明点击综合审查即主动发起、双重脱敏、模型服务商处理范围，以及照片和附件原文件不发送。
...
- 普通 AI 问答不发送合同全文；用户可逐项选择合同审查、验房、证据包和补贴模块的业务摘要，并在发送前预览。
- 合同页点击一次“开始综合审查”后先在本地运行规则，合同文字经本机和服务端双重脱敏、服务端分段，再交给模型全文复核；结果区不再提供重复的 AI 解读按钮。
- AI 补充风险必须携带可回查的合同逐字原文；无证据结果会被丢弃，再与本地规则结果去重合并。
- AI 复核失败或取消时保留本地规则结果；合同正文不写盘、不进入服务端日志或幂等缓存。
