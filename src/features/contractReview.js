...
function getLeaseFindings(text) {
  ...
  add(makeLeaseFinding({
    ...
    matchGroups: [
      ['市场行情', '市场情况', '周边租金', '周边房价', '周边房价行情', '房价行情', '经营需要', '平台调价'],
      ['调整租金', '提高租金', '上浮租金', '变更租金', '上调房租', '上调', '涨价'],
      ['不接受', '不同意', '乙方不同意', '拒绝调整', '拒绝涨价', '不补缴', '拒不补缴', '未补缴'],
    ],
  ...
  add(makeLeaseFinding({
    ...
    matchGroups: [
      ...
      ['换锁', '更换门锁', '停水停电', '强制收房', '强行收房', '自行收回房屋', '合同直接终止', '合同终止', '租赁合同终止'],
    ],
  ...
  add(makeLeaseFinding({
    ...
    minHits: 99,
    matchGroups: [
      ['晾晒衣物', '粘贴挂钩', '挂钩贴纸', '贴贴纸', '留宿', '饲养宠物', '噪音投诉', '被投诉噪音'],
      ['扣除押金', '从押金扣除', '每次扣除押金', '每次罚款', '每次扣款'],
    ],
  ...
  add(makeLeaseFinding({
    ...
    minHits: 99,
    matchGroups: [
      ['甲方出售', '甲方卖房', '出售房屋', '抵押房屋', '收回房屋', '要求租客搬走', '要求乙方搬离'],
